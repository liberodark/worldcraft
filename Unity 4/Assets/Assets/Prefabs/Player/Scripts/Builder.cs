...
public class Builder : MonoBehaviour {
	...
	public void SetSelectedBlock(Block block) {
		...
		if (this.networkView.isMine && Network.peerType != NetworkPeerType.Disconnected)
			networkView.RPC("SetSelectedBlockIndex", RPCMode.AllBuffered, block.Index);
	}
	...
	public void SetSelectedBlock(Item item) {
		...
		if (this.networkView.isMine && Network.peerType != NetworkPeerType.Disconnected)
			networkView.RPC("SetSelectedBlockIndex", RPCMode.AllBuffered, item.Index);
	}
	...
	void Update(){
		...
		if (selected is ActivableItem){
			if(InputManager.inputManager ().isItemActivated){
				...
				Camera.main.light.enabled = !Camera.main.light.enabled;
			}
		}else if(Camera.main.light.enabled)
	...
	{
		networkView.RPC("RemoteDestroyBlock", RPCMode.All, (Vector3)cursor_pos);
	}
	...
	{
		networkView.RPC("RemoteSpawnBlock", RPCMode.All, (Vector3)cursor_pos);
	}
